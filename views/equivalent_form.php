...
function run_local() {
    $(".act_button").mouseup(function(e) {
        ...
        if (toks[0] == "DEL") {
            ...
            window.location.href = base_url + "/editor/equivalent/del/" + toks[1]
        }
        if (toks[0] == "NEW") {
            //set a dedicated link
            window.location.href = base_url + "/editor/equivalent/new"
        }
    })
...
if ($list) {
    ...
    foreach($list as $item) {
        $c = 0;
        echo form_open("{$bu}/editor/equivalent/save",

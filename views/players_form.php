...
function run_local() {
    $(".act_button").mouseup(function(e) {
        ...
        if (toks[0] == "DEL") {
            ...
            window.location.href = base_url + "/editor/players/del/" + toks[1]
        }
        if (toks[0] == "NEW") {
            //set a dedicated link
            window.location.href = base_url + "/editor/players/new"
        }
    })
...
if ($list) {
    ...
    foreach($list as $item) {
        $c = 0;
        echo form_open("{$bu}/editor/players/save",
        ...
        foreach($item as $f=>$v) {
            ...
            if ($f == "ptype") {
                $html = form_dropdown($f,array("AI"=>"Art. intell.", "HU"=>"Human"),$v,$data);
            } else {
                $data["class"] = "editable";

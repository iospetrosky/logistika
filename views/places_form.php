...
function run_local() {
    $(".act_button").mouseup(function(e) {
        ...
        if (toks[0] == "DEL") {
            ...
            window.location.href = base_url + "/editor/places/del/" + toks[1]
        }
        if (toks[0] == "NEW") {
            //set a dedicated link
            window.location.href = base_url + "/editor/places/new"
        }
    })
...
if ($list) {
    foreach($list as $item) {
        ...
        foreach($item as $f=>$v) {
            ...
            if ($f == "ptype") {
                $html = form_dropdown($f,array("PLAINS"=>"Plains", "MOUNT"=>"Mountains"),$v,$data);
            } elseif ($f == 'major') {
                $html = form_dropdown($f,$majors,$v,$data);
            } else {
                //$data["class"] = "editable";
